...
# - 规则树页面
class handler:     
    @app_helper.ex_check_sign(['app_id','tick','page_id_or_code'], 'TNM') 
    def POST(self):
        ...
        if len(param['page_id_or_code'])<20: # _id长度为24
            ...
            real_page_id = param['page_id_or_code'] #app_helper.realid(param['page_id_or_code']) # 恢复原始id
        ...
        return json.dumps({'ret' : 0, 'data' : {
                'gid'        : str(r1['_id']), #app_helper.randomid(r1['_id']),
                'type'       : r1['type'],
                'title'      : r1['page_name'],
                'page_code'  : r1['page_code'],
                'start_node' : str(r1['start_node']), #app_helper.randomid(r1['start_node']),
                'text'       : r1['rich_text'],
            } 
        })
